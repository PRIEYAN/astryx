...
export const docs = {
  ...
  props: [
    ...
    vars: [
      ...
      {name: '--spinner-arc-fraction', description: "How much of the circle the moving arc covers, as a unitless fraction of the full ring: 0.375 is the default 135 degree sweep, 0.75 draws 270 degrees. It varies with neither size nor shade, so set it on the base target. The arc is composed from the diameter either way, so a themed sweep holds its proportion at every size. The dash the SVG carries as an attribute stays the default sweep, so a themed value applies from the frame the stylesheet does, not in a render that has none.", default: '0.375 (135 degrees)'},
    ],
  },
  usage: {
    description:
...
export const docsZh = {
  ...
  theming: {
    ...
    vars: [
      ...
      {name: '--spinner-arc-fraction', description: '运动圆弧覆盖整圈的比例，取值为不带单位的分数：0.375 是默认的 135 度扫过角度，0.75 则绘制 270 度。它既不随 size 也不随 shade 变化，因此请在 base 目标上设置。无论取值为何，圆弧都由直径合成，所以主题化的扫过角度在每个尺寸下都保持同样的比例。SVG 以属性形式携带的虚线仍为默认扫过角度，因此主题值从样式表生效的那一帧起才应用，在没有样式表的渲染中不会应用。', default: '0.375（135 度）'},
    ],
  },
  usage: {
    description:
